...
const getPrivateClient = (manager: typeof discordManager): any => (manager as any).client;
...
describe('DiscordManager', () => {
  ...
  describe('connect()', () => {
    ...
    it('should break early if Client.login rejects', async () => {
      ...
      expect(discordManager.connect()).rejects.toThrow('Invalid Token provided');
      expect(discordManager.isConnected()).toBe(false);
    });
  });
  ...
  describe('checkWhitelist()', () => {
    ...
    describe('With Loaded Guild Context', () => {
      ...
      it('strips "discord:" prefix out of input identifiers cleanly', async () => {
        mockMembersFetch.mockResolvedValue({ roles: { cache: mockRolesCache } });
      ...
      it('returns true if the extracted member possesses targeted whitelisted roles', async () => {
        ...
        expect(mockRolesCache.hasAny).toHaveBeenCalledWith(...mockConfig.whitelistedRoles);
