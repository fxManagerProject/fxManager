...
function createMockSocket(initialReadyState = 1): MockWebSocket {
  ...
  return {
    ...
    simulateClientEvent: (event: 'message' | 'close', data?: any) => {
      eventRegistry[event]?.forEach((cb) => { cb(data) });
    },
  };
}
...
describe('WSManager Suite', () => {
  ...
  describe('Client Lifecycle Management (add / remove)', () => {
    ...
    it('should ignore incoming payload strings that fail to match valid JSON formatting schemas', () => {
      ...
      expect(() => {
        mockSocket.simulateClientEvent('message', Buffer.from('NOT_A_VALID_JSON_STRING'));
      }).not.toThrow();
    });
  });
  ...
  describe('Subscription Routing and Authorization Hooks', () => {
    it('should allow channel subscriptions if no conditional connectionChecks are declared', async () => {
      ...
      const subscribeMessage: WSClientMessage = { type: 'subscribe', channel: 'playerlist' };
      mockSocket.simulateClientEvent('message', Buffer.from(JSON.stringify(subscribeMessage)));
    ...
    it('should enforce connection check rules and reject subscriptions on verification failures', async () => {
      ...
      wsManager.addCheck('console', (admin) => PermissionManager.has(admin.permissions, UserPermissions.CONSOLE_ACCESS));

      const subscribeAttempt: WSClientMessage = { type: 'subscribe', channel: 'console' };
      mockSocket.simulateClientEvent('message', Buffer.from(JSON.stringify(subscribeAttempt)));
    ...
    it('should securely fall back onto structural wildcard routing definitions (*)', async () => {
      ...
      const subscribeAttempt: WSClientMessage = { type: 'subscribe', channel: 'report:123' };
      mockSocket.simulateClientEvent('message', Buffer.from(JSON.stringify(subscribeAttempt)));
    ...
    it('should unsubscribe cleanly when receiving an unsubscribe event frame', async () => {
      ...
      const unsubscribeFrame: WSClientMessage = { type: 'unsubscribe', channel: 'server_state' };
      mockSocket.simulateClientEvent('message', Buffer.from(JSON.stringify(unsubscribeFrame)));
  ...
  describe('Data Providers Execution Contexts', () => {
    it('should pull, execute, and dispatch initial structural arrays from providers immediately upon subscribing', async () => {
      ...
      const msg: WSClientMessage = { type: 'subscribe', channel: 'resourcelist' };
      mockSocket.simulateClientEvent('message', Buffer.from(JSON.stringify(msg)));
    ...
    it('should fall back onto dynamic wildcard providers if an explicit map lookup is unavailable', async () => {
      ...
      expect(wildcardProviderMock).toHaveBeenCalledWith('client-id', 'report:55');
      expect(mockSocket.send).toHaveBeenCalledWith(JSON.stringify({
        channel: 'report:55',
        event: 'initial',
    ...
    it('should trap errors occurring inside a data provider safely without crashing the event loops', async () => {
      ...
      const msg: WSClientMessage = { type: 'subscribe', channel: 'server_state' };
      mockSocket.simulateClientEvent('message', Buffer.from(JSON.stringify(msg)));
  ...
  describe('Outbound Communication Framework (send / broadcast)', () => {
    it('should transmit messages to explicit targeting instances when socket states are active (readyState 1)', () => {
      ...
      const messagePacket = { channel: 'console' as Channel, event: 'line', data: 'Execution log string' };
      wsManager.send('client-target', messagePacket);

      expect(mockSocket.send).toHaveBeenCalledWith(JSON.stringify(messagePacket));
    });
    ...
    it('should suppress transmission if target destinations are not in an active open state', () => {
      ...
      wsManager.send('client-target', { channel: 'console', event: 'line', data: 'Dropped line frame' });
      expect(mockSocket.send).not.toHaveBeenCalled();
    });
    ...
    it('should broadcast items solely to active matching channel connections', () => {
      ...
      const broadcastMsg = { channel: 'perf' as Channel, event: 'snapshot', data: { cpu: 12 } };
  ...
  describe('Inbound Event Bus Bindings (on / onChannel)', () => {
    it('should trigger both specific handlers and wildcard listeners when an emit arrives', async () => {
      ...
      mockSocket.simulateClientEvent('message', Buffer.from(JSON.stringify(emitMessage)));

      await Promise.resolve();

      const expectedClientContext = (wsManager as any).clients.get('client-id');
      expect(exactHandler).toHaveBeenCalledWith(expectedClientContext, 'command', { command: 'ensure oxmysql' });
      expect(channelWildcardHandler).toHaveBeenCalledWith(expectedClientContext, 'command', { command: 'ensure oxmysql' });
    });
    ...
    it('should detach hooks immediately when invoking returned unbind closure handles', async () => {
      ...
      const detachClosure = wsManager.on('console', 'command', executionHandler);
    ...
    it('should route handlers via onChannel syntax structures perfectly as wildcard catchments', async () => {
      ...
      await Promise.resolve();
      expect(globalChannelHandler).toHaveBeenCalledWith(expect.any(Object), 'arbitrary_event', { id: 1 });
    });
  });
});
