...
export function useResourcelistSocket(): UseResourcelistReturn {
	...
	useEffect(() => {
		...
		const offSync = on<ResourceInitialData>('resourcelist', 'initial', ({ data }) => {
			setResources(data.resourcelist);
      setStatus(data.status);
			setLoading(false);
		});

		const offRefresh = on<ResourceInitialData>('resourcelist', 'refresh', ({ data }) => {
			setResources(data.resourcelist);
      setStatus(data.status);
			setLoading(false);
		});

		const offUpdate = on<ResourceData>(
			'resourcelist',
			'update',
			({ data }) => {
				setResources((prev) => {
